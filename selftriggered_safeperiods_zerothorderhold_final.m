...
current_timestep = [];
po = []; % min(Tcbf,Tclf) update interval
u_hold = 0;
tnext = 0;
for k = 1:total_k-1
    if t + dt >= tnext
...
xlabel('t')
ylabel('u')
title('Self triggered control u over time')
hold off
% 
% figure(2)
% grid on
...
xlabel('x2')
ylabel('x1')
title('Self triggered controller trajectory with ZOH')
hold off
% 
% figure(6)
% grid on
...
function clf = defineClf1(symbolic_state,x1d,x2d)
